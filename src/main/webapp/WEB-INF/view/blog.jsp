  ...
  <body>
		...
		<div class="row">
			<div class="col-md-8">
				...
				<div class="text-center">
					...
					浏览：${blog.hits}
					<a href="/toTouSu?aid=${blog.id }">投诉</a>
				</div>
				<hr/>
				<div class="content">
					${blog.content}
					...
					<c:forEach items="${comments}" var="comment">
						<div class="media">
						  ...
						  <div class="media-body">
						    ...
						    <p>${comment.content}</p>
						    <p>评论时间：
						    	<fmt:formatDate value="${comment.displayTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
						    </p>
						  </div>
						</div>
					</c:forEach>
					
					<input type="button" class="btn btn-info" value="首页" onclick="fenye(1)">
					<input type="button" class="btn btn-info" value="上一页" onclick="fenye(${pi.isIsFirstPage()?1:pi.getPrePage()})">
					<input type="button" class="btn btn-info" value="下一页" onclick="fenye(${pi.isIsLastPage()?pi.getPages():pi.getNextPage()})">
					<input type="button" class="btn btn-info" value="尾页" onclick="fenye(${pi.getPages()})">
				</div>
				<!-- 评论列表展示 -->
				
				<div>
					<!-- 添加评论  可以使用form表单提交    也可以使用ajax提交     -->
	...
	<script type="text/javascript">
		...
		function fenye(cpage){
			location="/article/${blog.id}?cpage="+cpage;
		}
